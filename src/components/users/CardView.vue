  ...
  <div>
    ...
    <div class="max-w-sm my-4 overflow-hidden bg-white rounded-lg shadow-lg">
      <div class="flex items-center px-6 py-3 bg-gray-900">
        ...
        <h1 class="mx-3 text-lg font-semibold text-white"> <router-link :to="{name: 'userId' , params: {id: user.id}}">{{user.name}}</router-link> </h1>
      </div>
      <div class="px-6 py-4">
        <h1 class="text-2xl font-semibold text-gray-800">ID: {{user.id}}</h1>
        ...
        <div class="flex items-center mt-4 text-gray-700">
          ...
          <h1 class="px-2 text-sm">{{user.username}}</h1>
        </div>
        <div class="flex items-center mt-4 text-gray-700">
          <svg class="w-6 h-6 fill-current" viewBox="0 0 512 512">
            <path
          ...
          <h1 class="px-2 text-sm">{{user.address.city}}</h1>
        </div>
        <div class="flex items-center mt-4 text-gray-700">
          <svg class="w-6 h-6 fill-current" viewBox="0 0 512 512">
            <path
